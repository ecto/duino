  ...
  this.detect(function (err, serial) {
    ...
    setTimeout(function(){
      ...
      if (self.debug) {
        ...
        self.serial.write('!99' + normalizePin(0) + normalizeVal(1) + '.');
        process.on('SIGINT', function(){
          self.log('info', 'sending debug mode toggle off to board');
          self.serial.write('!99' + normalizePin(0) + normalizeVal(0) + '.');
          delete self.serial;
          setTimeout(function(){
            process.exit();
          }, 100);
...
Board.prototype.digitalRead = function (pin) {
  ...
  this.write('02' + pin + normalizeVal(0));
}
...
Board.prototype.analogRead = function (pin) {
	...
	this.write('04' + pin + normalizeVal(0));
}
