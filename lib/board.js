...
/*
 * Tell the board to read from RF receiver
 */
Board.prototype.rfRead = function (pin) {
  pin = this.normalizePin(pin);
  this.log('info', 'rfRead from pin ' + pin);
  this.write('05' + pin + this.normalizeVal(0));
}

/*
 * Tell the board to send RF message
 */
Board.prototype.rfTransmit = function (pin, msg) {
  pin = this.normalizePin(pin);
  this.log('info', 'rfTransmit from pin ' + pin, 'with message: ' + msg);
  this.write('06' + pin + msg);
}
